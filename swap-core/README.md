...
2. **Re-check the timelocks with the current time.** `validateMessage` enforces a stateless floor (time-based CLTV, `T1 − T2 ≥ 2h`), but not T2 recency. Run **`checkAcceptAgainstQuote(quote, accept, nowSec)`** (binds `quote_id` + `recv_sats` + `validateTimelocks`) before funding, and pass `t1/t2/nowSec` to `verifyMakerContract`/`verifyTakerContract`.
